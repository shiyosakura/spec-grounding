...
export async function POST(req: NextRequest) {
  ...
  // Calculate nomination fee
  const nominationFee = is_nominated === 1 ? NOMINATION_FEE : 0;
  ...
  const result = db
    ...
    .run(customer_id, staff_id, is_nominated, nominationFee, start_datetime, totalDuration, now, now);
  ...
  return NextResponse.json({ reservation_id: reservationId, nomination_fee: nominationFee }, { status: 201 });
}
