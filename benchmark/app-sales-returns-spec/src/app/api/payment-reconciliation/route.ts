...
export async function POST(request: NextRequest) {
  try {
    ...
    // Credit notes (status=4) have negative invoice_amount — they act as offsets, skip unreconciled check
    for (const entry of validEntries) {
      const invoice = db
        .prepare("SELECT invoice_amount, status FROM invoices WHERE id = ?")
        .get(entry.invoice_id) as { invoice_amount: number; status: number } | undefined;
      ...
      // Skip unreconciled balance check for credit notes (status=4)
      if (invoice.status === 4) continue;
    ...
    const processReconciliation = db.transaction(() => {
      ...
      // Credit notes (status=4) are not updated — they retain status=4
      const affectedInvoiceIds = [...new Set(validEntries.map((e: { invoice_id: number }) => e.invoice_id))];

      for (const invoiceId of affectedInvoiceIds) {
        const invoice = db
          .prepare("SELECT invoice_amount, status FROM invoices WHERE id = ?")
          .get(invoiceId) as { invoice_amount: number; status: number };

        // Skip credit notes — they retain status=4
        if (invoice.status === 4) continue;
