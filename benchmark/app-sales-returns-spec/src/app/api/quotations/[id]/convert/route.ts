  ...
  try {
    ...
    const result = db.transaction(() => {
      ...
      if (customer.credit_limit >= 1) {
        ...
        // Sum billing_amount from outstanding invoices (status 1=Issued, 2=Partially Paid, or 4=Credit Note)
        // Credit notes (status=4) have negative invoice_amount, so they reduce the outstanding balance
        const billingTotal = db
          .prepare(
            `SELECT COALESCE(SUM(invoice_amount), 0) as total
             FROM invoices
             WHERE customer_id = ? AND status IN (1, 2, 4)`
          )
