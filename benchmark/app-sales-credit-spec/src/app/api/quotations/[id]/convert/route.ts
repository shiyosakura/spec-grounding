  ...
  try {
    ...
    const result = db.transaction(() => {
      ...
      if (customer.credit_limit >= 1) {
        ...
        // This transaction's order amount
        ...
        if (outstandingTotal > customer.credit_limit) {
          // Credit limit exceeded: rollback order and restore quotation status
          db.prepare("DELETE FROM order_items WHERE order_id = ?").run(orderId);
          db.prepare("DELETE FROM orders WHERE id = ?").run(orderId);
          db.prepare("UPDATE quotations SET status = 1, updated_at = ? WHERE id = ?").run(now, quotationId);
      ...
      return { orderId, orderNumber, creditWarning, rejected: false };
    })();

    if (result.rejected) {
      return NextResponse.json(
        { success: false, error: "Credit limit exceeded." },
        { status: 400 }
      );
    }
