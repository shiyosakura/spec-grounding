...
export async function GET(
  ...
  try {
    ...
    // Calculate outstanding_balance dynamically
    const taxRate = getTaxRate();

    // (1) Unpaid invoices: sum of invoice_amount minus reconciled amounts for status IN (1, 2)
    const unpaidInvoiceBalance = db
      .prepare(
        `SELECT COALESCE(SUM(
          inv.invoice_amount - COALESCE((
        ...
        FROM invoices inv
        WHERE inv.customer_id = ? AND inv.status IN (1, 2)`
      )
      .get(customerId) as { total: number };

    // (2) Confirmed but not fully billed orders (status IN (0, 1, 2))
    // An order is "not fully billed" if it has no invoice_items or not all order_items are covered
    const unbilledOrders = db
      .prepare(
        `SELECT o.id FROM orders o
         WHERE o.customer_id = ? AND o.status IN (0, 1, 2)
         AND NOT EXISTS (
           SELECT 1 FROM invoice_items ii
           INNER JOIN order_items oi ON ii.order_item_id = oi.id
           INNER JOIN invoices inv ON ii.invoice_id = inv.id
           WHERE oi.order_id = o.id AND inv.status IN (1, 2, 3)
         )`
      )
    ...
    const outstandingBalance = unpaidInvoiceBalance.total + unbilledOrderTotal;
