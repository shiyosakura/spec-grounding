...
import { getDb, generateNumber, getTaxRate, calculateVolumeDiscount } from "@/lib/db";
...
export async function POST(
  ...
  try {
    ...
    const result = db.transaction(() => {
      ...
      const orderInfo = db
        .prepare(
          `INSERT INTO orders (order_number, customer_id, quotation_id, subject, status, credit_warning, discount_rate, discount_amount, ordered_at, updated_at)
           VALUES (?, ?, ?, ?, 0, 0, 0, 0, ?, ?)`
        )
      ...
      // Volume Discount: Calculate and apply based on pre-tax subtotal
      const orderSubtotalForDiscount = db
        .prepare(
          `SELECT COALESCE(SUM(quantity * unit_price), 0) as subtotal FROM order_items WHERE order_id = ?`
        )
        .get(orderId) as { subtotal: number };
      const { discountRate, discountAmount } = calculateVolumeDiscount(orderSubtotalForDiscount.subtotal);
      if (discountRate > 0) {
        db.prepare(
          `UPDATE orders SET discount_rate = ?, discount_amount = ? WHERE id = ?`
      ...
      // === Credit Check Phase (§3-9 steps 6-7) ===
      const customer = db
      ...
      if (customer.credit_limit >= 1) {
        ...
        const unbilledOrders = db
          ...
          .all(quotation.customer_id, orderId) as { id: number }[];
        ...
        // This transaction's order amount (after discount)
        const thisOrderSubtotal = db
          .prepare(
            `SELECT COALESCE(SUM(quantity * unit_price), 0) as subtotal FROM order_items WHERE order_id = ?`
          )
          .get(orderId) as { subtotal: number };
        const thisOrderAfterDiscount = thisOrderSubtotal.subtotal - discountAmount;
        const thisOrderTax = Math.floor((thisOrderAfterDiscount * taxRate) / 100);
        const thisOrderTotal = thisOrderAfterDiscount + thisOrderTax;
