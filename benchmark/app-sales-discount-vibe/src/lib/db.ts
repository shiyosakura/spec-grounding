...
function initializeSchema(db: Database.Database): void {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS orders (
      ...
      CHECK (discount_amount >= 0 AND discount_amount <= 99999999),
      FOREIGN KEY (customer_id) REFERENCES customers(id)
    );
...
/**
 * Calculate volume discount based on pre-tax subtotal.
 * - 500,000 or more: 10% discount
 * - 100,000 or more: 5% discount
 * - Otherwise: no discount
 * Returns { discountRate, discountAmount } where discountAmount is floored.
 */
export function calculateVolumeDiscount(subtotal: number): {
  discountRate: number;
