...
export async function POST(req: NextRequest) {
  ...
  // Handle modification source: cancel old reservation with fee = 0, no penalty
  if (modification_source_reservation_id && modification_source_reservation_id > 0) {
    db.prepare(
      "UPDATE reservations SET status = 1, cancellation_fee = 0, updated_at = ? WHERE reservation_id = ?"
