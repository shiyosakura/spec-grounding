...
function ReserveContent() {
  ...
  const [cancellationPolicy, setCancellationPolicy] = useState<CancellationPolicyTier[]>([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/cancellation-policy").then(r => r.json()).then(setCancellationPolicy);
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full">
            ...
            <div className="space-y-2 text-sm mb-4">
              ...
              <p><span className="text-gray-500">Total:</span> &yen;{totalPrice.toLocaleString()} / approx. {totalDuration} min</p>
              <div>
                ...
                <ul className="ml-4 mt-1 text-xs text-gray-600">
                  {cancellationPolicy.map((tier, i) => {
                    const nextThreshold = cancellationPolicy[i + 1]?.hour_threshold;
                    let label: string;
                    if (i === 0) {
                      label = `Up to ${tier.hour_threshold} hours before: ${tier.cancellation_rate}%`;
                    } else {
                      const prevThreshold = cancellationPolicy[i - 1].hour_threshold;
                      label = `${prevThreshold}-${tier.hour_threshold} hours before: ${tier.cancellation_rate}%`;
                    }
                    return <li key={tier.tier_id}>{label}</li>;
                  })}
                  {cancellationPolicy.length > 0 && (
                    <li>{cancellationPolicy[cancellationPolicy.length - 1].hour_threshold}+ hours before: Free</li>
                  )}
                </ul>
              </div>
            </div>
