...
function initializeSchema(db: Database.Database) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS reservations (
      ...
      cancellation_fee_rate INTEGER NOT NULL DEFAULT 0 CHECK(cancellation_fee_rate >= 0 AND cancellation_fee_rate <= 100),
  ...
  insertSetting.run("cancellation_tier1_hours", "72");
  insertSetting.run("cancellation_tier1_rate", "0");
  insertSetting.run("cancellation_tier2_hours", "24");
  insertSetting.run("cancellation_tier2_rate", "50");
  insertSetting.run("cancellation_tier3_rate", "100");
  insertSetting.run("noshow_fee_rate", "100");
