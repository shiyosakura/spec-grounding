  ...
  try {
    ...
    const processAll = db.transaction(() => {
      for (const customer of targetCustomers) {
        ...
        // Step 3: Update customer outstanding_balance
        db.prepare(
          `UPDATE customers
           SET outstanding_balance = MIN(outstanding_balance + ?, 99999999)
           WHERE id = ?`
        ).run(invoiceAmount, customer.id);

        // Step 4: Update order status to "invoiced" (3) where all order items are now billed
