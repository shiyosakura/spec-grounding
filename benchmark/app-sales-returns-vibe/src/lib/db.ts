...
function initializeSchema(db: Database.Database): void {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS customers (
      ...
      CHECK (outstanding_balance >= 0 AND outstanding_balance <= 99999999)
    );
    ...
    CREATE TABLE IF NOT EXISTS returns (
      ...
      FOREIGN KEY (shipping_instruction_id) REFERENCES shipping_instructions(id),
      FOREIGN KEY (customer_id) REFERENCES customers(id)
    );
    ...
    CREATE TABLE IF NOT EXISTS credit_notes (
      ...
      issue_date TEXT NOT NULL,
      registered_at TEXT NOT NULL DEFAULT (datetime('now', 'localtime')),
      CHECK (credit_amount >= 0 AND credit_amount <= 99999999),
    ...
    CREATE TABLE IF NOT EXISTS credit_note_reconciliations (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      credit_note_id INTEGER NOT NULL,
      invoice_id INTEGER NOT NULL,
      reconciled_amount INTEGER NOT NULL,
...
function runMigrations(db: Database.Database): void {
  // Migration: Add outstanding_balance to customers if not present
  const customersInfo = db.prepare("PRAGMA table_info(customers)").all() as Array<{ name: string }>;
  const hasOutstandingBalance = customersInfo.some((col) => col.name === "outstanding_balance");
  if (!hasOutstandingBalance) {
    db.exec(`ALTER TABLE customers ADD COLUMN outstanding_balance INTEGER NOT NULL DEFAULT 0`);
  }

  // Migration: Add return_number_prefix and credit_note_number_prefix settings if not present
  const returnPrefixSetting = db
    .prepare("SELECT value FROM system_settings WHERE key = 'return_number_prefix'")
    .get();
  if (!returnPrefixSetting) {
    db.prepare("INSERT INTO system_settings (key, value) VALUES ('return_number_prefix', 'RET-')").run();
  }

  const cnPrefixSetting = db
    .prepare("SELECT value FROM system_settings WHERE key = 'credit_note_number_prefix'")
    .get();
  if (!cnPrefixSetting) {
    db.prepare("INSERT INTO system_settings (key, value) VALUES ('credit_note_number_prefix', 'CN-')").run();
  }
}
...
function seedIfEmpty(db: Database.Database): void {
  ...
  const seedAll = db.transaction(() => {
    ...
    insertSetting.run("return_number_prefix", "RET-");
    insertSetting.run("credit_note_number_prefix", "CN-");
