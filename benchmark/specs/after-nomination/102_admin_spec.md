...
- Nomination fee: If `Reservation[selected_reservation_id].nomination_fee` > 0, display "Nomination fee: ¥XXX"
...
- Total price: Sum of `Reservation Menu Detail.price_at_booking` + `Reservation[selected_reservation_id].nomination_fee` in "¥X,XXX" format
