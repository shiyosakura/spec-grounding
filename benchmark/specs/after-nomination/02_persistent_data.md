...
| | cancellation_penalty_count | 0–10 | 1 | Cumulative count of same-day cancellations. Booking is blocked when this reaches `cancellation_penalty_limit` |
...
| | nomination_fee | 0–99999 | 1 | Nomination fee (yen). Set to `System Settings.nomination_fee` (default `500`) when `is_nominated` = 1. Set to 0 when `is_nominated` = 0. Recorded at reservation creation |
