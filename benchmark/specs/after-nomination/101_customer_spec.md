...
1. Create a new record in `Reservation`.
   ...
   - `nomination_fee`: If `is_nominated` = 1, set to `System Settings.nomination_fee` (default `500`). If `is_nominated` = 0, set to 0
...
- Calculate and display the total price:
  ...
  - If `selected_staff_id` >= 1 (nominated), add `System Settings.nomination_fee` (default `500`)
  - Store the result in `total_price` and display in "¥X,XXX" format
  - If nomination fee is included, display "(includes ¥500 nomination fee)" as a note
...
- For each reservation in the upcoming reservation list, display the following:
  ...
  - Nomination fee: If `Reservation[reservation_id].nomination_fee` > 0, display "Nomination fee: ¥XXX"
  - Total price: Sum of `Reservation Menu Detail.price_at_booking` + `Reservation[reservation_id].nomination_fee` in "¥X,XXX" format
...
- Recalculate and display the following when `selected_menu_list` changes:
  - Total price: Sum of `Menu Master[selected_menu_list[i]].price` + nomination fee (if `selected_staff_id` >= 1, `System Settings.nomination_fee`) -> Store in `total_price`
...
- Nomination fee: If `selected_staff_id` >= 1, display "Nomination fee: ¥`System Settings.nomination_fee`"
- Total price: `total_price` in "¥X,XXX" format (includes nomination fee if applicable)
