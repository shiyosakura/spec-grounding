...
- Retrieve `Customer Master[customer_id].credit_limit` for the target customer.
  ...
  - If `credit_limit` ≥ 1 (credit check applies) ⇒ Calculate the outstanding balance total and proceed to credit judgment.
    ...
    - If outstanding balance total exceeds `Customer Master[customer_id].credit_limit` ⇒ **Reject the order. Roll back all changes made in the Order Conversion Phase (delete the newly created `Order Data`, `Order Line Items`, and revert `Quotation Data.status` to its previous value). Return an error response (HTTP 400) with a message indicating that the credit limit has been exceeded. Do not proceed to Order Confirmation.**
...
7. Calculate the outstanding balance total.
   ...
   - If the total exceeds `Customer Master[customer_id].credit_limit` ⇒ **Reject the order and roll back: delete `Order Line Items` created in step 3, delete `Order Data` created in step 2, revert `Quotation Data[edit_target_quotation_id].status` to 1 (Submitted), revert `Quotation Data[edit_target_quotation_id].updated_at` to the value before step 5. Return an error response (HTTP 400) with a message indicating that the credit limit has been exceeded (e.g., "Credit limit exceeded. Outstanding: ¥[outstanding balance total], Limit: ¥[credit_limit]"). Do not proceed to step 8.**
