...
> Note: When invoices are fully paid through reconciliation (step 2, status → 3), the corresponding invoice amounts are no longer included in the customer's outstanding balance calculation used by the credit check in `101_order_spec.md` §3-9. This effectively releases the credit limit capacity, allowing the customer to place new orders up to their credit limit again. No explicit "credit release" operation is needed — it is an automatic consequence of the outstanding balance formula excluding fully paid invoices.
