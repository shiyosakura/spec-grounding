...
2. Calculate and include `outstanding_balance` as a computed field in the customer detail response.
   - `outstanding_balance` = Sum of `Invoice.Invoice Amount` from invoices of this customer where `Invoice.Status` = 1 (Issued) or 2 (Partially Paid), minus the sum of already-reconciled amounts (Σ `Payment Reconciliation.Reconciled Amount` for those invoices) + Sum of tax-inclusive order amounts from `Order Data` of this customer where `Order Data.status` IN (0, 1, 2) (Confirmed, Shipping, Shipped) and the order has not yet been fully invoiced (i.e., at least one `Order Line Item` is not yet registered in `Invoice Line Item`). Tax-inclusive amount per order = tax-exclusive subtotal (Σ(`Order Line Item.quantity` × `Order Line Item.unit_price`)) + tax amount (tax-exclusive subtotal × `System Settings.consumption_tax_rate` ÷ 100, rounded down to the nearest yen).
   - This field is read-only and not stored in the database. It is computed on each retrieval.
3. Proceed to display update (§4-2).
...
- Display the following in each row of the customer list table.
  ...
  - Outstanding Balance: The computed `outstanding_balance` from §3-7 step 2, displayed in "¥XX,XXX,XXX" format. Display "¥0" when the customer has no outstanding obligations. This field is displayed in the customer detail view (individual customer retrieval) and is not required in the list view.
- Customer edit form:
  - If `Target Customer ID` = 0 ⇒ Display an empty form (new addition mode)
  - If `Target Customer ID` ≥ 1 ⇒ Display a form preset with the values from `Customer Master[Target Customer ID]` (edit mode). Include `outstanding_balance` as a read-only display field.
