...
1. Retrieve records from `Invoice` under the following conditions.
   ...
   - If `Filter Status` = 3 (Fully Paid) ⇒ Retrieve records where `Invoice.Status` = 3.
   - If `Filter Status` = 4 (Credit Note) ⇒ Retrieve records where `Invoice.Status` = 4.
...
2. Retrieve records from `Invoice` where `Customer ID` matches the Customer ID retrieved in step 1, and `Status` = 1 (Issued) or `Status` = 2 (Partially Paid) or `Status` = 4 (Credit Note).
   - Credit Notes (Status = 4) are included so they can be applied as offsets against normal invoices during reconciliation.
3. For each `Invoice`, retrieve records from `Payment Reconciliation` where `Invoice ID` matches, and calculate the reconciled amount (Σ `Payment Reconciliation.Reconciled Amount`) (calculated in UI Hook; see §4-5).
   - For Credit Notes: the `Invoice Amount` is negative, and the "unreconciled balance" is also negative (representing credit available to offset).
...
- Display the following in each row of the invoice list table.
  ...
  - Status: Display the label corresponding to `Invoice.Status` as a badge (0=Unissued, 1=Issued, 2=Partially Paid, 3=Fully Paid, 4=Credit Note)
