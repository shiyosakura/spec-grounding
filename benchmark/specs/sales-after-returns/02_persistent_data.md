...
| | Invoice Number | String | 1 | For normal invoices: `System Settings.Invoice Number Prefix` + sequential number. For credit notes: `System Settings.Invoice Number Prefix` + "CN-" + sequential number |
| | Customer ID | 1-9999 | 1 | Reference ID for `Customer Master` |
| | Billing Period | String | 1 | YYYY-MM format. The year and month covered by this invoice |
| | Invoice Amount | -99999999〜99999999 | 1 | In yen. For normal invoices: positive value — total invoice amount confirmed at time of issuance (snapshot of `Σ(Invoice Line Item.Quantity × Invoice Line Item.Unit Price) + Tax Amount`). For credit notes: **negative value** = −(Σ(Return Item.Quantity × original unit_price) + Σ(floor(Return Item.Quantity × original unit_price × `System Settings.Consumption Tax Rate` / 100) per Return Item row)). Does not change after issuance |
| | Status | 0-4 | 1 | 0 = not issued, 1 = issued, 2 = partially paid, 3 = paid in full, 4 = credit note |
| | Return ID | 0-999999 | 1 | 0 = not a credit note (normal invoice). 1 or above = reference ID for the `Return` that generated this credit note. Only set when Status = 4 |
...
| Return | Return ID | 0-999999 | 1 | 0 = none (reserved value). Auto-incremented. Valid IDs are 1 and above |
| | Shipping Instruction ID | 1-999999 | 1 | Reference ID for `Shipping Instruction`. Identifies the shipment being returned against |
| | Customer ID | 1-9999 | 1 | Reference ID for `Customer Master` (copied from `Shipping Instruction`) |
| | Credit Note Invoice ID | 0-999999 | 1 | 0 = credit note not yet issued. Reference ID for the `Invoice` (Status=4) auto-generated on return registration |
| | Created At | DateTime | 1 | |
| Return Item | Return Item ID | 0-999999 | ×30 | 0 = unused slot. Maximum 30 lines per return |
| | Return ID | 0-999999 | ×30 | Reference ID for `Return` |
| | Product ID | 1-9999 | ×30 | Reference ID for `Product Master` |
| | Quantity | 1-99999 | ×30 | Return quantity. Must satisfy: cumulative return quantity for the same Shipping Instruction + Product ≤ shipped quantity |
| | Unit Price | 0-9999999 | ×30 | In yen. Copied from the original `Order Line Item.Unit Price` (the price at the time of order) |
