...
| Invoice List Screen | Filter Status | -1〜4 | 1 | -1 = show all, 0 = not issued, 1 = issued, 2 = partially paid, 3 = paid in full, 4 = credit note |
...
| Returns Registration Screen | Active Shipping Instruction ID | 1-999999 | 1 | ID of the `Shipping Instruction` passed from the Shipping Instruction List Screen. Must have status = 2 (shipped) |
| | Return Quantity Input | 0-99999 | ×30 | Return quantity input per shipping instruction line item row. 0 = no return for this row. Initial value 0 |
