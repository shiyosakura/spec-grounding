...
2. Set `Reservation[selected_reservation_id].cancellation_fee` to the sum of `Reservation Menu Detail[selected_reservation_id].price_at_booking` x 100 / 100 (= 100% of total price).
3. Update `Reservation[selected_reservation_id].updated_at` to current datetime.
4. Increment `Customer[Reservation[selected_reservation_id].customer_id].cancellation_penalty_count` by 1 (not exceeding `System Settings.cancellation_penalty_limit`).
5. Re-execute fetch admin reservations (§3-1) to refresh the list.

> Note: No-show charges a 100% cancellation fee (full amount). The penalty count is also incremented.
...
- Cancellation fee: If `Reservation[selected_reservation_id].status` is 1 (cancelled) or 4 (no-show) and `Reservation[selected_reservation_id].cancellation_fee` > 0 => Display "Cancellation fee: ¥X,XXX"
