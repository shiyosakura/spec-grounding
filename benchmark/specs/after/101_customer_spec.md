...
> Referenced Data:
> - `01_master_data.md` (Menu Category, Menu Master, Staff Master, Staff Shift, Staff Menu Assignment, Cancellation Policy, System Settings)
...
| §2-9, §3-9 | Cancellation eligibility check / fee calculation | On cancel button press | My Page |
...
- Retrieve the record from `Staff Shift` matching `selected_staff_id` x day of week of `selected_date`.
  - If `is_working` = 0 (day off) => Set all slots of `available_slot_staff_ids` to 0 (unused), display "The selected staff member is off on this day." End process.
...
### §2-9. Cancellation Eligibility Check / Fee Calculation

Executed when the cancel button is pressed on the My Page screen.

> Note: My Page is accessible only to logged-in customers. Cancellation of guest customer (`account_id` = null) reservations is handled by phone only — the admin manually changes the status via the reservation calendar in `102_admin_spec.md`.
...
- Calculate the remaining time (in hours) from the current datetime to `Reservation[target_reservation_id].start_datetime`.
- Scan `Cancellation Policy` in ascending order of `hour_threshold` (smallest first), and apply the `cancellation_rate` of the first tier where remaining time < `hour_threshold`. If no tier matches (remaining time >= the largest hour_threshold) => cancellation_rate = 0%.
  > Example: With default 3 tiers (24h/100%, 72h/50%, infinity/0%), 12h remaining => "12 < 24" matches first, 100% applied. 48h remaining => "48 < 24" fails -> "48 < 72" matches, 50%. 100h remaining => none match, 0%.
- `cancellation_fee_preview` = sum of `Reservation Menu Detail[target_reservation_id].price_at_booking` x `cancellation_rate` / 100 (rounded down).
- Store the applied rate in `cancellation_rate_preview`.
- Proceed to data retrieval (§3-9) -> Display the cancellation confirmation dialog. After confirmation, proceed to cancellation processing (§3-10).
...
- If `cancellation_rate_preview` > 0 => Proceed to data update with penalty increment (§3-10).
- If `cancellation_rate_preview` = 0 => Proceed to data update without penalty increment (§3-10).
...
6. Store the results in `available_slot_start_times` and `available_slot_staff_ids`. Set `available_slot_staff_ids` to `selected_staff_id`.
7. If no valid slots are available (all `available_slot_staff_ids` are 0) => Display "No availability found for the selected criteria."
...
5. Store the consolidated results in `available_slot_start_times` and `available_slot_staff_ids`. Set `available_slot_staff_ids` to the auto-assignment candidate's staff ID.
6. If no valid slots are available (all `available_slot_staff_ids` are 0) => Display "No availability found for the selected criteria."
...
1. Search `Reservation` for records matching the following conditions:
   - `staff_id` = target staff ID (`available_slot_staff_ids[selected_time_slot]`)
   - `status` = 0 (confirmed)
...
1. Create a new record in `Reservation`.
   ...
   - `staff_id`: `available_slot_staff_ids[selected_time_slot]`
...
3. If `modification_source_reservation_id` >= 1 (modification mode) => Update `Reservation[modification_source_reservation_id].status` to 1 (cancelled), set `Reservation[modification_source_reservation_id].cancellation_fee` to 0, and update `updated_at` to current datetime.
   > Note: This cancellation of the source reservation is an automatic process accompanying a reservation modification and does NOT increment `Customer.cancellation_penalty_count`. The `cancellation_fee` is also set to 0. The status value uses the same 1 as a regular cancellation.
   > Note: If the new reservation is subsequently cancelled normally, the cancellation fee is calculated based on the new reservation's `start_datetime`. Even if the original reservation was for today, if the new reservation is 72+ hours away, it qualifies for free cancellation. This is an accepted behavior based on the design principle that "reservation modification is a customer-favorable operation."
...
### §3-9. Cancellation Eligibility Check / Fee Calculation

This process does not update data. It updates the UI based on the check/calculation results from §2-9.

- Cancellation not allowed (penalty limit reached) => Display error message.
- Cancellation allowed => Display the cancellation confirmation dialog with the following:
  - Cancellation rate: `cancellation_rate_preview` in "XX%" format
  - Cancellation fee: `cancellation_fee_preview` in "¥X,XXX" format
  - If `cancellation_fee_preview` = 0 => Display "No cancellation fee will be charged"
  - If `cancellation_fee_preview` > 0 => Display "A cancellation fee of ¥X,XXX (XX%) will be charged"
...
2. Set `Reservation[target_reservation_id].cancellation_fee` to the value of `cancellation_fee_preview`.
3. Update `Reservation[target_reservation_id].updated_at` to current datetime.
4. If penalty increment was determined in §2-10 (`cancellation_rate_preview` > 0) => Increment `Customer[customer_id].cancellation_penalty_count` by 1 (not exceeding `System Settings.cancellation_penalty_limit`).
5. Re-fetch the reservation list (§3-8).
...
- Update the time slot list based on `available_slot_start_times`. Display only slots where `available_slot_staff_ids` >= 1, showing each slot's start time in "HH:MM" format.
- If no valid slots are available (all `available_slot_staff_ids` are 0), display "No availability."
- Total price: Sum `Menu Master[menu_id].price` for each menu ID in `selected_menu_list` and display in "¥X,XXX" format.
- Calculate and display the total duration as follows:
...
- For each reservation in the upcoming reservation list, display the following:
  ...
  - Cancellation fee: If `Reservation[reservation_id].status` = 1 (cancelled) and `Reservation[reservation_id].cancellation_fee` > 0 => Display "Cancellation fee: ¥X,XXX"
...
- Recalculate and display the following when `selected_menu_list` changes:
  - Total price: Sum of `Menu Master[selected_menu_list[i]].price` in "¥X,XXX" format
...
- Staff: `Staff Master[available_slot_staff_ids[selected_time_slot]].staff_name`. If `selected_staff_id` = 0, display "Auto-assigned: [staff_name]"
...
- Cancellation policy: List all tiers of `Cancellation Policy`. For each tier: "Up to [hour_threshold] hours before: [cancellation_rate]%" format. Example: "3+ days before: Free / 1-3 days before: 50% / Same day: 100%"
